 ...
 *
 * CLAIM-THEN-ACT (AC2 correctness): each def is claimed via resolveEscalationDef FIRST - an atomic
 * `UPDATE ... WHERE resolved = false` that returns true only for the caller that flipped it. If a
 * concurrent /acknowledge (or an overlapping cycle / second instance) already resolved it, the
 * claim returns false and we skip - so an alert acknowledged a moment before the deadline never
 * escalates, and the same def never double-fires.
 *
...
export async function runEscalationCycle(limit = 50): Promise<EscalationCycleResult> {
  ...
  for (const def of due) {
    try {
      // Claim first: only proceed if THIS cycle flipped resolved false -> true.
      const claimed = await resolveEscalationDef(def.source_event_id);
      if (!claimed) continue;
      ...
      // Task 4.4: record the hop as a domain event, not only a read-model row.
      await persistEvent({
      ...
      }).catch((err) => console.error(`Persisting notification.escalated event failed for ${def.source_event_id}:`, err));
