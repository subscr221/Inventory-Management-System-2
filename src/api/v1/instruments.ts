...
import { findMatchingDoaEntry, findRoleHolder } from '../../read/projections/doa_registry.js';
    ...
    const event = await persistEvent(
      {
        ...
        payload: {
          ...
          previous_status: before.calibration_status,
...
const createCalibrationEscalationBase: RouteHandler = async (req, res, params) => {
  ...
  const status = await getInstrumentCalibrationStatus(instrumentId);
  if (!status) {
  ...
  const entry = await findMatchingDoaEntry('calibration.escalation', 0);
  if (!entry) {
