...
| 2.7 | Run `npm run db:migrate` inside the app container; confirm it is re-runnable (second run, no error) | migrate log, two clean runs |
| 2.8 | Rehearsal-only operator client: `deploy/provision/keycloak-add-cli-client.sh` adds `ims-cli` (direct access grant) to the live realm so operator scripts can take a token with `deploy/provision/ims-token.sh <email>`; delete the client when Story 1.12 (edge UI sign-in) ships. Rotate the bootstrap admin password in the Keycloak console | client visible in the realm |
...
| 2.9a | DOA bands: `deploy/provision/staging-doa-bands.sh <finance-controller-email>` registers the three approval bands (`migration.domain_signoff` for `department_head`, `migration.variance_explanation` for `finance_controller`, `jobwork.offcut_acquisition` for `cfo`) as the finance controller, then `verify:roles` | `All segregated role pairs are provisioned on separate users.` |
| 2.10 | End to end: a token for the migration lead through `https://auth.ancorlabs.org` and `GET /api/v1/migration/golive/reconciliation?site_id=` through `https://ims-staging.ancorlabs.org`; an empty report with `gate.blocking: APPROVAL_REQUIRED` proves auth, roles, database and proxy | report JSON |
| 2.11 | SSH hardening, only now: create a named operator account with the automation key, then set `PermitRootLogin prohibit-password` and `PasswordAuthentication no` in sshd and reload. Not before the operator account has logged in once | `sshd -T` output |
| 2.12 | Backups: confirm pgBackRest (`deploy/backup/backup.sh`) has produced one full backup and one WAL archive segment; for staging the local repository is acceptable, production needs an off-box copy | backup listing |
