...
| 2.15 | Deploy by hand, ruled 2026-09-22: the CD job `deploy-staging` is switched off (repository variable `STAGING_RUNNER_ENABLED` unset) because the self-hosted runner cannot start on the shared box. After CD `build-and-push` succeeds for the commit, ship the scripts with `git archive HEAD deploy/pipeline deploy/backup deploy/provision` piped over ssh into `/opt/ims`, then run `bash /root/staging-deploy-sha.sh <full sha>` (a copy is kept at `deploy/pipeline/staging-deploy-sha.sh`): it takes a logical dump to `/root/ims-db-pre-<sha7>-<ts>.dump`, runs `deploy.sh staging` (which migrates) and restarts nginx. Take a labelled `staging-basebackup.sh` before and after | `Health check passed (new release)` and the two images on the new sha |
