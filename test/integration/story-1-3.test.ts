...
describe('Story 1.3 Integration Tests', () => {
  ...
  before(async () => {
    ...
    const auditLogSql = readFileSync(
      resolve(__dirname, '../../read/projections/audit_log.sql'),
      'utf-8',
    );
  ...
  it('AC1: event write produces an audit entry with trace_id, user_id, role, location_id, timestamp, endpoint, method, event_id, seq_no', async () => {
    ...
    const res = await makeRequest(
      ...
      'POST',
      '/api/v1/events',
      makeEventPayload('inventory', streamId),
      authHeaders,
    );
  ...
  it('AC1: unauthenticated POST is rejected with 401 and no audit entry is created', async () => {
    ...
    const body = auditRes.body as Record<string, unknown>;
    const entries = body['entries'] as Array<Record<string, unknown>>;
    const hasEntryForUnauthenticated = entries.some(
      (e) => e['event_id'] === null && e['http_status'] === 401,
    );
    assert.strictEqual(
      hasEntryForUnauthenticated,
      false,
      'No audit entry should exist for unauthenticated request',
    );
  });
  ...
  it('AC2 (Task 2.5): a direct DB DELETE via the admin pool is rejected by the tamper trigger', async () => {
    ...
    await makeRequest(
      ...
      'POST',
      '/api/v1/events',
      makeEventPayload('inventory', '00000000-0000-0000-0000-0000000000a1'),
      authHeaders,
    );
  ...
  it('AC2 (Task 2.5): a direct DB UPDATE via the admin pool is rejected by the tamper trigger', async () => {
    await makeRequest(
      ...
      'POST',
      '/api/v1/events',
      makeEventPayload('inventory', '00000000-0000-0000-0000-0000000000a2'),
      authHeaders,
    );
  ...
  it('AC2: no REST route exists to delete an audit log entry', async () => {
    ...
    const entries = (auditRes.body as Record<string, unknown>)['entries'] as Array<
  ...
  it('AC3: auditor query returns entries in append order (monotonic seq_no) with a range digest and no gaps', async () => {
    ...
    assert.strictEqual(
      sequenceCheck['append_order_verified'],
      true,
      'seq_no should be strictly increasing',
    );
    assert.strictEqual(
      sequenceCheck['gap_check_applicable'],
      true,
      'gap check applies to an un-user-filtered query',
    );
    assert.strictEqual(
      sequenceCheck['is_contiguous'],
      true,
      'no committed rows should be missing from the range',
    );
  ...
  it('AC3 (P2): cursor pagination returns every entry exactly once with no drops or duplicates', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await makeRequest(
        ...
        'POST',
        '/api/v1/events',
        makeEventPayload('inventory', `00000000-0000-0000-0000-0000000000b${i}`),
        authHeaders,
      );
    }
    ...
    const fullRes = await makeRequest(
      TEST_PORT,
      'GET',
      `/api/v1/audit/log?${auditRange}&limit=1000`,
      undefined,
      authHeaders,
    );
    const fullEntries = (fullRes.body as Record<string, unknown>)['entries'] as Array<
      Record<string, unknown>
    >;
    ...
    for (let guard = 0; guard < 1000; guard++) {
      ...
      const pageRes: { status: number; body: Record<string, unknown> } = await makeRequest(
        TEST_PORT,
        'GET',
        url,
        undefined,
    ...
    assert.deepStrictEqual(
      pagedIds,
      fullIds,
      'paged log_ids must equal the full ordered set (no drops, no dupes)',
    );
    assert.strictEqual(
      new Set(pagedIds).size,
      pagedIds.length,
      'no duplicate log_ids across pages',
    );
  });
  ...
  it('AC3: a valid-but-unmatched user_id filter returns zero entries and marks gap check inapplicable', async () => {
    ...
    assert.strictEqual(
      sequenceCheck['gap_check_applicable'],
      false,
      'gap check must be inapplicable under a user filter',
    );
    assert.strictEqual(
      sequenceCheck['is_contiguous'],
      null,
      'is_contiguous must be null under a user filter',
    );
  });
  ...
  it('AC3 (P6): a non-UUID user_id is rejected with 400 INVALID_PARAMS', async () => {
    const res = await makeRequest(
      TEST_PORT,
      'GET',
      `/api/v1/audit/log?${auditRange}&user_id=not-a-uuid`,
      undefined,
      authHeaders,
    );
  ...
  it('AC3 (P6): an invalid start_date is rejected with 400 INVALID_PARAMS', async () => {
    const res = await makeRequest(
      TEST_PORT,
      'GET',
      `/api/v1/audit/log?start_date=nonsense&end_date=${WIDE_END}`,
      undefined,
      authHeaders,
    );
    assert.strictEqual(res.status, 400);
  ...
  it('AC3 (P6): a non-positive limit is rejected with 400 INVALID_PARAMS', async () => {
    const res = await makeRequest(
      TEST_PORT,
      'GET',
      `/api/v1/audit/log?${auditRange}&limit=-5`,
      undefined,
      authHeaders,
    );
    assert.strictEqual(res.status, 400);
  ...
  it('AC3 (P6): a non-numeric cursor is rejected with 400 INVALID_PARAMS', async () => {
    const res = await makeRequest(
      TEST_PORT,
      'GET',
      `/api/v1/audit/log?${auditRange}&cursor=abc`,
      undefined,
      authHeaders,
    );
    assert.strictEqual(res.status, 400);
  ...
  it('AC4: enabling the audit log returns 200 and the mutating request is itself edit-logged (P10)', async () => {
    ...
    assert.strictEqual(
      rows.rows[0]!['event_id'],
      null,
      'a non-event mutation carries a null event_id',
    );
  });
  ...
  it('Task 6.3 (P1): a SCIM provisioning event writes a system-actor audit entry (role "system", location "*", status 201)', async () => {
    ...
    assert.strictEqual(
      scimEntry!['user_id'],
      SYSTEM_ACTOR_ID,
      'SCIM audit actor should be the system principal',
    );
    assert.strictEqual(
      scimEntry!['location_id'],
      '*',
      'SCIM audit location should be the system-wide marker',
    );
    assert.strictEqual(
      scimEntry!['http_status'],
      201,
      'provisioning audit row records the 201 the client received',
    );
  ...
  it('audit log entry user_id matches authenticated caller, not forged value', async () => {
    ...
    const actor = (eventResult['metadata'] as Record<string, unknown>)['actor'] as Record<
      string,
      unknown
    >;
    assert.notStrictEqual(
      ...
      'forged user_id should be overwritten',
    );
    ...
    const auditEntries = (auditRes.body as Record<string, unknown>)['entries'] as Array<
      Record<string, unknown>
    >;
    ...
    assert.strictEqual(
      auditRow!['user_id'],
      callerUserId,
      'audit user_id must equal the authenticated caller',
    );
    assert.strictEqual(
      auditRow!['role'],
      'warehouse_operator',
      'audit role must be the RBAC-authorized role, not the forged one',
    );
  });
  ...
  it('AC1 (P10): an idempotent SCIM no-op PATCH still produces an edit-log entry', async () => {
    ...
    const first = await makeRequest(
      TEST_PORT,
      'PATCH',
      '/api/v1/scim/v2/Users/noop-user-1-3@example.com',
      { active: false },
      scimHeaders,
    );
    ...
    assert.ok(
      rows.rows.length >= 2,
      'both the real deprovision and the no-op repeat must be edit-logged',
    );
    ...
    assert.strictEqual(
      noOpRow['http_status'],
      200,
      'the no-op records the 200 the client received',
    );
    ...
    assert.strictEqual(
      realRow['http_status'],
      200,
      'PATCH flows record 200, not a hardcoded 201 (P5)',
    );
  });
  ...
  it('AC5 (P7): archive CLI exports eligible rows, marks them exactly once, and re-runs are no-ops', async () => {
    ...
    assert.strictEqual(
      firstRun.archivedCount,
      2,
      'both 9-year-old rows are eligible; the recent rows are not',
    );
    assert.ok(firstRun.archivePath, 'an export file path is reported');
    const { readFileSync: readF, rmSync } = await import('node:fs');
    const exported = JSON.parse(readF(firstRun.archivePath!, 'utf-8')) as Array<
      Record<string, unknown>
    >;
    ...
    assert.strictEqual(
      Number(markers.rows[0]!['count']),
      2,
      'exactly one marker per archived row (P6)',
    );
    ...
    assert.strictEqual(
      Number(originals.rows[0]!['count']),
      2,
      'archived rows are still retrievable online (AC5)',
    );
