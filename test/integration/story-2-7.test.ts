...
import {
  ...
  runObsolescenceScan,
} from '../../src/compliance/planning-jobs.js';
...
describe('Story 2.7 Safety Stock, Reorder Points, and Obsolescence Flagging', () => {
  ...
  async function getParams(sku: string, locationId: string): Promise<Record<string, unknown>> {
    const res = await makeRequest(
      port,
      'GET',
      `/api/v1/planning/params/${sku}?location_id=${locationId}`,
      undefined,
      plannerHeaders,
    );
  ...
  it('AC1: the worked example computes safety stock = 20 and reorder point by the documented formula', async () => {
    ...
    await postStockEvent(
      'stock.issued',
      { sku, target_location_id: locAId, quantity: 10 },
      daysAgoIso(5),
    );
    await postStockEvent(
      'stock.issued',
      { sku, target_location_id: locAId, quantity: 18 },
      daysAgoIso(3),
    );
    ...
    const compute = await makeRequest(
      ...
      { sku, location_id: locAId, business_date: BUSINESS_DATE },
      plannerHeaders,
    );
    ...
    assert.strictEqual(
      ...
      146,
      'reorder point = (avg * lead) + safety stock',
    );
    ...
    assert.strictEqual(
      inputs['lead_time_source'],
      'seeded',
      'lead_time_source recorded on every computation',
    );

    // Reproducible from its recorded inputs.
    const reproduced = Math.ceil(
      Number(inputs['z']) *
        Number(inputs['sigma_daily']) *
        Math.sqrt(Number(inputs['lead_time_days'])),
    );
    assert.strictEqual(
      reproduced,
      Number(row['safety_stock']),
      'the stored computation reproduces from computation_inputs',
    );
  });
  ...
  it('AC1: computation is idempotent - recomputing over unchanged demand emits no duplicate event', async () => {
    ...
    const again = await makeRequest(
      ...
      'POST',
      '/api/v1/planning/safety-stock/compute',
      { sku: 'SS-AC1', location_id: locAId, business_date: BUSINESS_DATE },
      plannerHeaders,
    );
    ...
    assert.strictEqual(
      after,
      before,
      'a recompute over unchanged state emits no new computation event',
    );
  });
  ...
  it('AC1: fails closed with INSUFFICIENT_DEMAND_HISTORY when the window has too few sample days', async () => {
    ...
    await postStockEvent(
      'stock.issued',
      { sku, target_location_id: locAId, quantity: 5 },
      daysAgoIso(2),
    );
    await setParams({
      sku,
    ...
    assert.strictEqual(
      row['safety_stock'],
      null,
      'no computation is stored on insufficient history',
    );
  });
  ...
  it('AC2: an on-hand at/below reorder point creates exactly one open recommendation and one planner alert', async () => {
    ...
    const check = await makeRequest(
      port,
      'POST',
      '/api/v1/planning/replenishment/check',
      { sku, location_id: locAId, business_date: BUSINESS_DATE },
      plannerHeaders,
    );
    ...
    assert.strictEqual(
      recommended[0]!['recommended_order_qty'],
      100,
      'recommended qty is the standard order qty',
    );

    const list = await makeRequest(
      port,
      'GET',
      `/api/v1/planning/replenishment/recommendations?sku=${sku}&status=open`,
      undefined,
      plannerHeaders,
    );
    ...
    assert.strictEqual(
      await notificationCount(sku, 'replenishment_recommended'),
      1,
      'exactly one planner alert emitted',
    );

    // Idempotent per crossing: a re-check over unchanged state adds no second recommendation or alert.
    const recheck = await makeRequest(
      port,
      'POST',
      '/api/v1/planning/replenishment/check',
      { sku, location_id: locAId, business_date: BUSINESS_DATE },
    ...
    assert.strictEqual(
      (recheck.body['recommended'] as unknown[]).length,
      0,
      'no second recommendation on re-check',
    );
    assert.strictEqual(
      await eventCount('replenishment.recommended', sku),
      1,
      'still exactly one recommendation event',
    );
    assert.strictEqual(
      await notificationCount(sku, 'replenishment_recommended'),
      1,
  ...
  it('AC2 concurrency: two concurrent checks for the same grain create at most one open recommendation', async () => {
    ...
    const scope = {
      ...
      actor: { user_id: plannerUserId, role: 'inventory_planner', location_id: NO_LOCATION_UUID },
    };
    await Promise.all([runReplenishmentCheck(scope), runReplenishmentCheck(scope)]);

    const open = await getPool().query(
      `SELECT count(*)::int AS c FROM replenishment_recommendation WHERE sku = $1 AND status = 'open'`,
      [sku],
    );
    assert.strictEqual(
      open.rows[0]!['c'],
      1,
      'no more than one open recommendation after concurrent checks',
    );
    assert.strictEqual(
      await eventCount('replenishment.recommended', sku),
      1,
      'exactly one recommendation event',
    );
  });
  ...
  it('AC3: an item with no issues past the threshold is flagged aging, appears in the report, and triggers NRV testing without a write-down', async () => {
    ...
    await getPool().query(
      `INSERT INTO inventory_valuation (sku, quantity_on_hand, running_average_cost, carrying_value) VALUES ($1, 30, 5, 150)`,
      [sku],
    );
    await setParams({
      sku,
    ...
    const scan = await makeRequest(
      port,
      'POST',
      '/api/v1/planning/obsolescence/scan',
      { sku, location_id: locAId, business_date: BUSINESS_DATE },
      plannerHeaders,
    );
    ...
    const report = await makeRequest(
      port,
      'GET',
      `/api/v1/planning/obsolescence/report?location_id=${locAId}`,
      undefined,
      plannerHeaders,
    );
    ...
    assert.strictEqual(
      await notificationCount(sku, 'obsolescence_flagged'),
      1,
      'exactly one NRV-review alert emitted',
    );
    ...
    const nrvRows = await getPool().query(
      `SELECT count(*)::int AS c FROM inventory_valuation_nrv_adjustment WHERE sku = $1`,
      [sku],
    );
    assert.strictEqual(
      nrvRows.rows[0]!['c'],
      0,
      'planning writes no NRV adjustment row (the DOA-gated valuation seam owns write-downs)',
    );

    // Idempotent: re-scanning an already-aging grain emits no second flag or alert.
    const rescan = await makeRequest(
      port,
      'POST',
      '/api/v1/planning/obsolescence/scan',
      { sku, location_id: locAId, business_date: BUSINESS_DATE },
    ...
    assert.strictEqual(
      (rescan.body['flagged'] as unknown[]).length,
      0,
      'no second flag on re-scan',
    );
    assert.strictEqual(
      await eventCount('obsolescence.flagged', sku),
      1,
      'still exactly one flag event',
    );
    assert.strictEqual(
      await notificationCount(sku, 'obsolescence_flagged'),
      1,
  ...
  it('AC3: resumed issue activity within the threshold clears the flag', async () => {
    ...
    assert.strictEqual(scan.status, 200, JSON.stringify(scan.body));
    assert.strictEqual(
      (scan.body['cleared'] as unknown[]).length,
      1,
      'resumed activity clears the flag',
    );
    ...
    const rep = (report.body['reports'] as Array<Record<string, unknown>>).find(
      (r) => r['sku'] === sku,
    )!;
  ...
  it('AC3 concurrency: two concurrent scans for the same aging grain create at most one flag', async () => {
    ...
    const flags = await getPool().query(
      `SELECT count(*)::int AS c FROM obsolescence_flag WHERE sku = $1 AND status = 'aging'`,
      [sku],
    );
  ...
  it('review patch: direct events and edge uploads enforce planning payload location write access', async () => {
    ...
    const edge = await makeRequest(
      port,
      'POST',
      '/api/v1/edge/events',
      { ...payload, idempotency_key: `plan-edge-${randomUUID()}`, device_id: 'rugged-2-7' },
      mixedScopePlannerHeaders,
    );
  ...
  it('review patch: code-keyed owned issues feed safety-stock demand stats and concurrent computes do not duplicate events', async () => {
    ...
    await postStockEvent(
      'stock.issued',
      { sku, target_location_code: locACode, quantity: 10 },
      daysAgoIso(5),
    );
    await postStockEvent(
      'stock.issued',
      { sku, target_location_code: locACode, quantity: 18 },
      daysAgoIso(3),
    );
    await setParams({
      sku,
    ...
    assert.strictEqual(
      Number(row['safety_stock']),
      20,
      'code-keyed issues are counted in demand stats',
    );
    assert.strictEqual(
      await eventCount('inventory_planning.safety_stock_computed', sku),
      1,
      'concurrent computes emit exactly one event',
    );
  });
  ...
  it('review patch: only owned stock feeds replenishment and obsolescence decisions', async () => {
    ...
    assert.strictEqual(check.status, 200, JSON.stringify(check.body));
    assert.strictEqual(
      (check.body['recommended'] as unknown[]).length,
      1,
      'owned on-hand 1 crosses reorder even with consignment present',
    );

    await getPool().query(
      `UPDATE stock_balance SET last_issue_at = now() - INTERVAL '200 days' WHERE sku = $1 AND stock_class = 'consignment'`,
      [sku],
    );
    const scan = await makeRequest(
      port,
    ...
    assert.strictEqual(scan.status, 200, JSON.stringify(scan.body));
    assert.strictEqual(
      (scan.body['flagged'] as unknown[]).length,
      0,
      'consignment activity does not age owned stock',
    );
  });
  ...
  it('review patch: never-issued owned stock uses a fallback clock and returns real days_since_issue', async () => {
    ...
    await postStockEvent(
      'stock.received',
      { sku, target_location_id: locAId, quantity: 30, unit_cost: 5 },
      daysAgoIso(200),
    );
    await setParams({
      sku,
    ...
    assert.ok(
      Number(flagged[0]!['days_since_issue']) >= 180,
      'response carries real days_since_issue',
    );
  });

  it('review patch: rejects overflowing lead time and zero-quantity direct recommendations cleanly', async () => {
    const lead = await setParams({
      sku: 'SS-AC1',
      location_id: locAId,
      lead_time_days: 1000000,
      lead_time_source: 'seeded',
      service_level: 0.95,
      business_stream: 'production',
    });
