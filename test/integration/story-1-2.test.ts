...
async function provision(
  ...
  assert.equal(
    res.status,
    201,
    `provisioning failed for ${externalId}: ${JSON.stringify(res.body)}`,
  );
...
async function devToken(sub: string): Promise<string> {
  ...
  assert.equal(
    res.status,
    201,
    `dev-token issuance failed for ${sub}: ${JSON.stringify(res.body)}`,
  );
...
describe('Story 1.2 Integration Tests', () => {
  ...
  it('AC1: no Authorization header returns 401 UNAUTHORIZED', async () => {
    const res = await makeRequest(
      TEST_PORT,
      'GET',
      '/api/v1/events/inventory/bbbbbbbb-0000-0000-0000-000000000001',
    );
  ...
  it('AC4: read-only role attempting a write returns 403 FUNCTION_ACCESS_DENIED', async () => {
    ...
    await provision(externalId, [
      { role: 'auditor', module: 'inventory', functionScope: 'read', locationId: '*' },
    ]);
    const token = await devToken(externalId);
  ...
  it('AC6: deprovisioning via SCIM invalidates the existing session on the next request', async () => {
    ...
    const before1 = await makeRequest(
      TEST_PORT,
      'POST',
      '/api/v1/events',
      eventBody('inventory', SITE_A),
  ...
  it('Review D1: audit actor identity is bound to the authenticated caller, not the request body', async () => {
    ...
    assert.equal(res.status, 201);
    const actor = (res.body['metadata'] as Record<string, unknown>)['actor'] as Record<
      string,
      unknown
    >;
    assert.equal(
      actor['user_id'],
      userId,
      'actor.user_id must be overwritten with the authenticated user',
    );
    assert.equal(
      actor['role'],
      'store_assistant',
      'actor.role must be the authorizing role, not the forged value',
    );
  });
  ...
  it('Review D3: stream reads are location-scoped to the caller grants', async () => {
    ...
    await provision(writerExt, [
      { role: 'admin', module: 'inventory', functionScope: 'write', locationId: '*' },
    ]);
    ...
    await provision(readerExt, [
      { role: 'viewer', module: 'inventory', functionScope: 'read', locationId: SITE_A },
    ]);
    ...
    const scopedActor = (scopedEvents[0]!['metadata'] as Record<string, unknown>)[
      'actor'
