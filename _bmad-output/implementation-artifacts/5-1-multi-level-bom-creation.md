...
- [ ] Task 2: Register event payloads and the engineering stream (AC: 1, 3, 8)
  ...
  - [ ] Register all three at the tail of `SUPPORTED_EVENT_TYPES` with `streamType: 'engineering'` and `requiresBusinessStream: true`. `engineering` is a new stream type value; add it without altering any existing entry or its order.
  ...
  - [ ] The handler injects the governed `business_stream` read from the parent `item_master` before `persistEvent`, mirroring the Story 4.4 and Story 4.7 pattern, because `assertInventoryTagging` runs before the transaction seam. The seam then re-derives it under lock and rejects any payload disagreement.
...
- `src/events/schema.ts:1045-1333`: append three interfaces and three registry entries after the Story 4.4 procurement block. Preserve every existing event name and its order.
