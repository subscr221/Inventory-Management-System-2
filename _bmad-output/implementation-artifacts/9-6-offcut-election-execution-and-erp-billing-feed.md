...
- [ ] [Review][Decision] Nothing reads `job_work_offcut_holding`, so retained offcut is invisible to the Story 9.5 breach sweep and the ageing report. The revised EPIC dev note places that widening in 9.6; this story's Reversal Notes defer it to 9.7; the code does neither. MITIGATED: because `reconcileReturnClocks` is correctly not called at capture, the exposure still ages on `jobwork_return_clock`. The round table ruled for deferring to 9.7 (the reader gets rewritten there anyway, so building it now builds it twice) - but the epic and the story still disagree in writing, and that needs fixing either way.
- [ ] [Review][Decision] Offcut capture is guarded by nothing but `has_contractual_offcut` and a plain `jobwork:write` role, so any shop-floor writer can drain a customer's entire remaining custody balance in one unreviewed posting. Under the revised model this is a separately-contracted customer asset whose value is set later, making it the highest-value posting in Epic 9 and the least controlled. Whether it warrants a second signature or a per-posting ceiling is a business policy call, not an engineering one.
