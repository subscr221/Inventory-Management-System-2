...
- `src/events/store.ts` (modified - fixed idempotency/stream-conflict constraint detection to use `err.constraint` instead of the never-matching `err.detail.includes(...)` check. Also, beyond the auth/RBAC scope of this story, tightened the Story 1.1 event-write contract: `occurred_at` now requires an explicit timezone offset via a strict ISO-8601 regex (rejecting tz-less timestamps Story 1.1 accepted via `Date.parse`); added optional client-supplied `event_version` optimistic-concurrency yielding `STREAM_CONFLICT`; expanded envelope validation for `event_version`/`schema_version`/`idempotency_key`; and made `persistEvent` an explicit `BEGIN`/`COMMIT` transaction. The timezone-mandatory contract is intentional for offline edge producers, which must emit unambiguous timestamps. Story 1.1's integration tests were updated to match.)
...
- 2026-07-13: Applied 18 fixes from adversarial code review (see Review Findings). Security-critical: the event audit actor (`user_id`/`role`) is now bound to the authenticated caller server-side rather than trusted from the request body (`events.ts`, `rbac.ts`, `context.ts`); authentication now runs before body parsing (`router.ts`); local auth mode fails closed unless `NODE_ENV` is explicitly `development`/`test` (`config/index.ts`); the SCIM bearer token is compared in constant time (`scim.ts`). Also: stream reads are now location-scoped to the caller's grants (`events.ts`); SCIM provisioning writes the directory row and roles in one transaction and deprovision/reactivation are idempotent (`users.ts`, `adapters/iam/scim.ts`); SCIM PATCH supports reactivation and rejects ambiguous bodies (`scim.ts`); JWT algorithms are pinned with a clock-skew allowance and case-insensitive `Bearer` scheme (`auth.ts`, `scim.ts`); RBAC rejects an unresolved module (`rbac.ts`); `DB_PORT` gains a NaN guard (`config/index.ts`); the compose app service declares required auth env (`docker-compose.yml`); the stray `diff_group1.txt` was removed; and the duplicated users DDL is cross-referenced for drift (`init-db.sql`, `read/projections/users.sql`). Added 6 regression tests for the new behaviors. Verified: `tsc --noEmit` and `eslint` clean, and all 21 integration tests (7 Story 1.1 + 14 Story 1.2) pass against real PostgreSQL 18.4.

## Review Findings

Adversarial code review on 2026-07-13 (baseline `7c60e01` to `HEAD`; layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor). Severity in brackets reflects consequence for this compliance-critical system, not the reviewing agent's rating.

Decision needed (resolve before patching):

- [x] `[Review][Decision]` [HIGH] Event `actor` identity is client-supplied and never reconciled with the authenticated caller - `postEventBase` writes `metadata.actor.user_id` and `actor.role` verbatim from the request body; RBAC only checks `actor.location_id`, and `validateEnvelope` only checks UUID/string format. A user with legitimate write access at their own location can forge the `user_id` and `role` recorded in the immutable event log, defeating the story's core attribution/non-repudiation goal. Decide: reject when `actor.user_id` does not match the token identity, or overwrite it server-side, and how to bind `actor.role` to the caller's actual roles. [src/api/v1/events.ts:29-34, src/events/store.ts:96-104] - RESOLVED 2026-07-13: overwrite `actor.user_id` from the authenticated token identity server-side; convert to patch.
- [x] `[Review][Decision]` [MEDIUM] Undocumented Story 1.1 event-store contract changes shipped under this story - beyond the documented `err.constraint` fix, `store.ts` now mandates a timezone on `occurred_at` (strict ISO-8601 regex, rejecting timestamps Story 1.1 accepted), adds client-supplied `event_version` optimistic concurrency, and expands envelope validation. None are in this story's File List or Change Log. Decide: keep and document (confirm the timezone-mandatory contract is intended for offline edge producers), or split them out. [src/events/store.ts:65-118, 150-159] - RESOLVED 2026-07-13: keep and document in File List and Change Log; timezone-mandatory contract confirmed intended.
- [x] `[Review][Decision]` [MEDIUM] Reads are not location-scoped - `getStreamHandler` passes no location resolver, so a user with `read` on a module receives every event in a stream regardless of the location each event occurred at. This matches the spec's deliberate "no location resolver on reads" decision, but is a real cross-location read exposure. Confirm it is acceptable for the pilot, or add scoping now. [src/api/v1/events.ts:60-63, src/middleware/rbac.ts:54-66] - RESOLVED 2026-07-13: add location scoping now; filter returned events to the caller's permitted locations for the module (wildcard-aware). Convert to patch.
- [x] `[Review][Decision]` [MEDIUM] SCIM PATCH semantics gaps - reactivation via `{active:true}` returns 400 (a deprovisioned user can only be re-enabled by re-POSTing); a combined `{active:false, roles}` silently drops the role change; a non-boolean `active` (e.g. `"false"`) is not treated as deactivation. Decide the required behavior for the organization's IdP-driven PATCH flows. [src/api/v1/scim.ts:93-106] - RESOLVED 2026-07-13: full fix - support `{active:true}` reactivation, reject ambiguous combined bodies with 400, validate the `active` type. Convert to patch.

Patch (unambiguous fixes):

- [x] `[Review][Patch]` [MEDIUM] Authenticate before parsing the body so protected body-bearing routes return 401 (not 400/413) for unauthenticated callers and do not buffer up to 10MB pre-auth [src/api/router.ts:69-95]
- [x] `[Review][Patch]` [MEDIUM] Make local auth mode fail closed: reject `AUTH_MODE=local` unless `NODE_ENV` is explicitly `development`, and default `.env.example` to `oidc`; today a copied `.env.example` on a host without `NODE_ENV=production` exposes the public dev-token endpoint, letting anyone mint a session for any known subject [src/config/index.ts:14-21, .env.example:12-17]
- [x] `[Review][Patch]` [MEDIUM] Compare the SCIM bearer token with `crypto.timingSafeEqual`, not `!==` (non-constant-time) [src/api/v1/scim.ts:9-15]
- [x] `[Review][Patch]` [MEDIUM] Make deprovision idempotent: add `AND active = true` to the `deactivateUser` UPDATE so repeat or concurrent deprovision does not append duplicate `user.deprovisioned` events to the audit stream [src/read/projections/users.ts:72-79]
- [x] `[Review][Patch]` [MEDIUM] Make SCIM provisioning and role replacement atomic: the directory write and its audit event span three separate transactions, with a concurrent delete-then-insert role race and no compensation if the event fails after the projection commits [src/adapters/iam/scim.ts:37-99, src/read/projections/users.ts:49-69]
- [x] `[Review][Patch]` [LOW] Pin accepted JWT algorithms (RS-family for oidc, HS256 for local) as defense in depth [src/middleware/auth.ts:40-48]
- [x] `[Review][Patch]` [LOW] Add a small JWT `clockTolerance` so valid tokens are not rejected at the `exp`/`nbf` boundary under normal clock drift [src/middleware/auth.ts:41-47]
- [x] `[Review][Patch]` [LOW] RBAC should deny an unresolved module (empty string) or unresolved location rather than skip the location check or let a `*` role satisfy an unknown module; currently safe only because `validateEnvelope` runs afterward [src/middleware/rbac.ts:36,54, src/api/v1/events.ts:7-23]
- [x] `[Review][Patch]` [LOW] Accept the `Bearer` auth scheme case-insensitively per RFC 7235 in both user auth and SCIM [src/middleware/auth.ts:27, src/api/v1/scim.ts:12]
- [x] `[Review][Patch]` [LOW] Guard that `payload.sub` is a string before using it as the directory key, and add a NaN guard to `DB_PORT` for parity with `PORT`/`DB_POOL_MAX` [src/middleware/auth.ts:45, src/config/index.ts:44]
- [x] `[Review][Patch]` [LOW] Deduplicate the `users`/`user_role_assignments` DDL defined in both `read/projections/users.sql` and `deploy/compose/init-db.sql` (grants live only in the latter) to prevent schema drift between the migrate and cluster-init paths [read/projections/users.sql, deploy/compose/init-db.sql]
- [x] `[Review][Patch]` [LOW] The production compose `app` service omits `AUTH_*` and `SCIM_BEARER_TOKEN`, so the container crash-loops on startup; add them or document required overrides [deploy/compose/docker-compose.yml:10-19]
- [x] `[Review][Patch]` [LOW] Remove the accidentally committed `diff_group1.txt` (36KB scratch blob) from the repository [diff_group1.txt]
- [x] `[Review][Patch]` [LOW] Route and allowlist edge cases: a trailing slash on `/api/v1/health` or `/api/v1/auth/dev-token` is treated as protected, and an `externalId` containing `/` can be provisioned but never PATCHed [src/api/router.ts:20-24,47]

Dismissed as noise (recorded, not actioned): duplicate-externalId POST reactivation (by design - `upsertUser` is an intentional upsert and the only reactivation path); minor identifiers echoed in 4xx error envelopes (the generic 500 leak was already removed); the `user_role_assignments` grant listing INSERT/SELECT/DELETE instead of the subtask's stated UPDATE (DELETE is correct for the delete-plus-reinsert pattern; only the subtask wording was imprecise).
