...
- [ ] [Review][Decision] `active_boms` and `open_pos` verification is not site-scoped, and the findings are readable by any site's migration lead - `computeActiveBoms` and `computeOpenPos` carry no `site_id` predicate (`bom` and `erp_purchase_order` have no site column, Open Question 1). With no `document_ref_prefix` a run for site A returns every legacy kit and every open PO line in the enterprise as `missing_in_source`, and `GET /verification-runs/:run_id` then serves those other-site kit refs and PO numbers to a site-A reader who passes `requireSiteReadAccess(site A)`. This is the 2026-09-06 cross-site class. Options: (a) make `document_ref_prefix` mandatory for these two domains and enforce it, (b) add a site column to the two projections (out of this story's scope), (c) accept enterprise scope for the single-site pilot and gate it before the second site. HIGH.
- [ ] [Review][Decision] Platform-only referential problems quarantine the domain unfixably - `referentialFindings` and `referential` run on platform rows that are not in the manifest, and the orphan sweeps emit `unknown_reference` keyed on a raw `service_order_id`. One pre-existing job-work receipt at the site with a deactivated SKU produces an unwaivable quarantine on every future run; `app_user` has no DELETE, so there is no in-system remedy and the domain can never be signed off. The integration test only escapes by deleting the row through the admin pool. Options: (a) emit `unknown_reference` only for documents present in the manifest and report platform-only reference problems as `missing_in_source` detail (waivable), (b) keep the block and add an operational fix route. Affects `migration_domain_verification.ts:537-557,638-658`. HIGH.
- [ ] [Review][Decision] `migrated_count` excludes any document with any finding, not just quarantined ones - `migration_domain_verification.ts:703` counts a document as migrated only when it has zero findings, so a PO line whose sole defect is a waivable tolerance mismatch is excluded exactly like a quarantined one, and the count is frozen at run time so waiving never restores it. AC 2 ties exclusion to quarantine. Decide whether "migrated" means "matched with no finding" (current) or "matched and not quarantined". MEDIUM.
- [ ] [Review][Patch] A second sign-off of an already-verified run overwrites the certification [src/compliance/migration-documents.ts:379-511,538-541]
- [ ] [Review][Patch] Delimiter-free composite keys silently drop findings and falsely reject manifest rows [src/read/projections/migration_domain_verification.ts:90; src/api/v1/migration.ts:1335; src/compliance/migration-documents.ts:118]
- [ ] [Review][Patch] `findings_sha256` and the four counts are accepted unvalidated by the pure shape assert [src/compliance/migration-documents.ts:126-158]
- [ ] [Review][Patch] SOD-07 loader check fails open when the import header row is absent [src/compliance/migration-documents.ts:431]
- [ ] [Review][Patch] The sign-off applier trusts the payload's signer identity instead of the envelope actor [src/compliance/migration-documents.ts:521-528]
- [ ] [Review][Patch] Custody verification does not filter `service_order` by site and overwrites duplicate keys unguarded [src/read/projections/migration_domain_verification.ts:578,594-597]
- [ ] [Review][Patch] Manifest import replay is keyed without the domain, so a reused key returns another domain's header [src/api/v1/migration.ts:1246,1432]
- [ ] [Review][Patch] A file whose rows are all rejected returns 201 with a `load_id` that addresses no manifest [src/api/v1/migration.ts:1359-1400]
- [ ] [Review][Patch] Mixed UOM in a custody register is masked by `min(e.uom)` over a summed balance [src/read/projections/migration_domain_verification.ts:574]
- [ ] [Review][Patch] Duplicate platform document keys are silently dropped instead of reported [src/read/projections/migration_domain_verification.ts:189,475]
- [ ] [Review][Patch] Waiving a quarantined finding returns 400 `INVALID_PARAMS` before the 409 `VERIFICATION_UNRESOLVED` that Task 8.3 specifies [src/compliance/migration-documents.ts:463-472]
- [ ] [Review][Patch] `document_ref_prefix` is unescaped in `LIKE`, absent from the payload type, and not persisted on the run header [src/read/projections/migration_domain_verification.ts:145-147; src/api/v1/migration.ts:1499-1512]
- [ ] [Review][Patch] `document_ref_prefix` is accepted and silently ignored for `jobwork_challans` and `custody_registers` [src/read/projections/migration_domain_verification.ts:683-687]
- [ ] [Review][Patch] Schema-drift guard weakened: `uq_migration_document_manifest_row_key` is declared but no longer pinned [test/unit/schema-drift.test.ts:2122-2128]
- [ ] [Review][Patch] The domain vocabulary has four hard-coded copies, and the finding-kind set two [src/migration/document-templates.ts:23; src/compliance/migration-opening-stock.ts:60; src/api/v1/migration.ts:1543]
- [ ] [Review][Patch] A negative custody balance cannot be expressed in the manifest (unsigned numeric regex) [src/migration/document-templates.ts:114]
- [ ] [Review][Patch] `migration_stage` rows for an unknown domain are dropped from `GET /migration/stages` [src/api/v1/migration.ts:814-826]
- [ ] [Review][Patch] The lot and GRN referential arms of `jobwork_challans` have no test coverage; no fixture is lot-controlled and every seeded receipt has `lot_id NULL` and a real `grn_line` [test/integration/story-13-2.test.ts:307,368]
- [ ] [Review][Patch] The Dev Agent Record understates what commit `42a568c` absorbed: it also carries all of Task 2 (the `MigrationDomain` unions, three payload/envelope pairs and three registry entries in `src/events/schema.ts`), not only the Task 1 schema files [this story file, Debug Log References]
...
| `deploy/compose/init-db.sql` | MODIFIED: mirrors |
| `src/events/migrate.ts` | MODIFIED: three tail entries |
| `src/events/schema.ts` | MODIFIED: `MigrationDomain` unions, three payload/envelope pairs, three registry entries |
| `src/compliance/migration-opening-stock.ts` | MODIFIED: event set, widened `import.completed` domain, assert delegation, three switch arms |
| `src/compliance/migration-documents.ts` | NEW: shape assert, three appliers, `assertDomainSignoffAllowed` |
| `src/migration/document-templates.ts` | NEW |
| `src/migration/opening-stock-template.ts` | MODIFIED: `MAX_IMPORT_ROWS`, `rowContentHash` |
| `src/read/projections/migration_domain_verification.ts` | NEW: `computeDomainFindings`, `findingsSha256`, `getDomainVerificationStatuses` |
| `src/api/v1/migration.ts` | MODIFIED: seven routes, `requireDomainSignoffActor`, stages default for all domains |
| `src/server.ts` | MODIFIED: route registrations |
| `src/cli/verify-segregated-roles-core.ts` | MODIFIED: `migration_lead` versus `department_head` pair |
| `src/adapters/erp/sync.ts` | MODIFIED: unused 13.1 constant removed |
| `test/unit/schema-drift.test.ts` | MODIFIED: three pins, widened-CHECK and ADD COLUMN fragment test |
| `test/integration/story-1-9.test.ts` | MODIFIED: spine allowlist |
| `test/integration/story-13-2.test.ts` | NEW: 20 tests |
| `docs/migration/document-manifest-templates-v1.md` | NEW |
| `docs/migration/opening-stock-template-v1.md` | MODIFIED: stage table rows |
| `_bmad-output/implementation-artifacts/deferred-work.md` | MODIFIED: five items |
