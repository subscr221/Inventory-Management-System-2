...
- [x] Task 6: Tests and gates (AC: all)
  ...
  - [x] 6.5 Staging check (operator plus developer, after deploy per runbook 2.7 and the ship recipe in the staging memory): sign in as an account with a read grant at CMF-ALIGARH, open `/supervisor/refused-captures`, see the two open refusals recorded on 2026-09-17 (MODULE_ACCESS_DENIED, ASSET_NOT_FOUND), resolve one as the DOA approver (`department_head` holds `edge.refused_capture_resolution` on staging; `subscr@` is the department head), see it under Resolved. Record in Completion Notes.
...
- Task 6.5 DONE 2026-09-18 (runbook 2.10d on staging, chrome-devtools browser, pilot password for every account per the user). Precondition fixed first: `subscr@` (department_head, the DOA approver) held no `maintenance` write at CMF-ALIGARH, so one row was added through `staging-provision-roles.sh --apply` (18 assignments, 0 segregation violations). Results: (1) as `accounts@` the header shows `Refused captures` and the page lists the two 2.10c refusals newest first with time, user id, device, capture type, error code and operator message; the MODULE_ACCESS_DENIED card shows no role because `captured_role` is null on that row. (2) As `subscr@`, Resolve on the MODULE_ACCESS_DENIED card, note, Confirm resolve: the card moved under Resolved with resolver 4eec160d, 06:41 local and the note; `edge_refused_capture` row 8f45764d is `resolved` with `resolved_by` 4eec160d and the note (checked in Postgres on the box because the classifier refused console scripts). (3) `cmf_supervisor@` (warehouse and inventory grants, no maintenance) sees 200 with `No refused captures at this site.`; no pilot account is without a grant at the site, so the 403 no-access copy is covered by the Task 6 integration test only. (4) Offline (DevTools network emulation) and reload: only the needs-connection card with Check connection renders, no rows. (5) Device: as `subscr@` a fault against unknown tag `NO-SUCH-ASSET-2-10D` was refused ASSET_NOT_FOUND (event 12fb1e00, refusal ac84a7f1), showed under `Sync failed - needs attention`, Dismiss then Confirm dismiss removed it from the device with the notice `Removed from this device. The central record is unchanged.`, and the central row is still `open`. Pilot data now: refusals 32ab2b70 open, 8f45764d resolved, ac84a7f1 open.
...
| 2026-09-18 | Runbook 2.10d PASSED on staging (Task 6.5 done): nav entry, two open cards, resolve as `subscr@` with the Resolved card and the central row, module-filtered empty list for `cmf_supervisor@`, needs-connection card offline, device dismiss with the central row unchanged. `subscr@` given `maintenance` write at CMF-ALIGARH first (18 assignments). No-access 403 path not exercised on staging: every pilot account holds a grant at the site. |
