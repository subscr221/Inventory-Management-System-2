---
baseline_commit: b9188aa17408bd88d45e5f1f54ababe61a9ab01a
---
...
- [x] Task 1: Location projection schema (AC: 1, 3)
  ...
  - [x] 1.2 Mirror the full DDL + guarded grants into `deploy/compose/init-db.sql` under a new `-- Event-sourced location (Story 1.6)` section, following the dual-file convention (Story 1.3 established, 1.4/1.5 followed): a banner comment at the top of both files stating they MUST stay identical. `deploy/compose/init-db.sql` is the first-boot copy, not the sole source of grants.
  - [x] 1.3 Register `../../read/projections/location.sql` in `src/events/migrate.ts`'s `MIGRATIONS` array, after `business_stream_config.sql`.

- [x] Task 2: Location invariant enforcement at the write path (AC: 1, 2)
  - [x] 2.1 Implement `src/compliance/location.ts` exporting `assertLocationInvariant(envelope: EventEnvelope): Promise<void>` (async because it reads the projection to compare asserted vs expected). This is the single enforcement point for AD-15. It throws `AppError` on a hard violation and returns `void` on success. Logic:
...
- `node --env-file=.env.test --import tsx --test test/unit/location.test.ts` - red phase failed until `src/compliance/location.ts` existed, then passed 6/6.
- `node --env-file=.env.test --import tsx --test test/unit/*.test.ts` - passed 21/21.
- `npx tsc --noEmit` - passed clean.
- `npm run lint` - passed clean.
- `node --env-file=.env.test --import tsx --test --test-concurrency=1 test/integration/story-1-6.test.ts` - passed 8/8.
...
- Implemented the canonical `read/projections/location.sql` schema with guarded grants, mirrored it in `deploy/compose/init-db.sql`, and registered it in `src/events/migrate.ts` after `business_stream_config.sql`.
- Added `src/read/projections/location.ts` for current-location, asserted-fact, and expected-fact projection reads and atomic upserts.
- Added `src/compliance/location.ts` as the AD-15 enforcement module, gated to `stream_type: 'inventory'` and `location.expected` / `location.asserted` event types only.
...
- `read/projections/location.sql` (new)
- `src/read/projections/location.ts` (new)
- `src/compliance/location.ts` (new)
- `src/api/v1/location.ts` (new)
- `src/events/store.ts` (modified)
- `src/events/migrate.ts` (modified)
- `src/server.ts` (modified)
- `deploy/compose/init-db.sql` (modified)
- `test/unit/location.test.ts` (new)
- `test/integration/story-1-6.test.ts` (new)
- `_bmad-output/implementation-artifacts/1-6-event-sourced-location-with-asserted-expected-separation.md` (modified)
