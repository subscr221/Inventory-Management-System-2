...
| **Date** | **Change** | **By** |
| --- | --- | --- |
| 2026-09-07 | Chunk D completion pass (bmad-code-review): D2/D5/D7/D8/D9 and the remaining D10 sub-arms all shipped - exact-code assertions replacing the false-greens (the `/api/v1/stock/issues` route arm was a phantom 404; AC 8 now truly closes its order; `offcutRetained` asserted exactly), the direct-events door arms (acquired finance gate, finance-gate site sub-check with a purpose-provisioned split-scope user, payload-site binding, currency guards, UNCITABLE/RETAINED/SUPERSEDED self-audited refusals, revaluation gate and posted_by pin on both doors), the ack-replay-after-grant-change, the arithmetic/lifecycle boundaries (SHORTFALL, disposal_id reuse, P9 zero-rounding, two-delta ack, the 22003 PROPOSE arm in the story-9-8 suite), the pointer-latest inverted-created_at arm and the schema-drift money/FK/clock-column pins, and the ageing horizon boundary. Exposed and fixed a real bug: `classifyDuplicate` did not classify the owned-lot/QC-hold constraint names, so a reused `disposal_id` returned a raw 500 instead of DUPLICATE_EVENT. Gates: tsc/eslint/prettier clean, story-9-7 38/38, story-9-8 13/13, story-1-9 6/6, schema-drift + predicates + registry 175/175. Status to done | code review |
...
- [x] [Review][Decision] AC 8's closed-order arm cannot fire when the clock is FULLY reconciled: the sweep-candidate query filters `status IN ('open','partially_reconciled')` before its closed-order EXISTS arm, and an over-tolerance receipt can fully absorb clock capacity while a retained holding stays outstanding past closure - the exact corner Story 9.5's non-strict reconcile was built for. The holding then ages silently (only the ageing report sees it). RULED 2026-09-07: admit closed-order retained holdings to the sweep independent of clock status. Fix applied: `listReturnClocksDueForSweep` now admits a `status = 'reconciled'` clock when the same (order, sku) still carries a retained holding, so the sweep never goes dark on a fully-reconciled-but-retained-offcut clock; ordinary reconciled clocks stay out of scope [jobwork_return_clock.ts:176-201]
- [x] [Review][Decision] The sweep's retained-offcut notice is (order, sku)-scoped and tells EVERY due clock of that sku that the full retained quantity is "already counted on this clock ... dispose of it to close the exposure" - false for every clock except the one that actually carries the quantity, and for offcut beyond challan capacity it is counted on no clock at all. RULED 2026-09-07: soften the notice text rather than plumb a clock-to-holding link. Fix applied: wording changed to "retained contractual offcut exists on this order, dispose or revalue it to close the exposure", dropping the per-clock ownership claim; story-9-7.test.ts AC 8 assertion updated to match [jobwork-clock-sweep.ts:170-188, story-9-7.test.ts:1639]

Post-ruling verification: tsc clean, story-9-7 30/30, story-9-5 34/34 (shared sweep query regression).
...
APPLIED 2026-09-06 (chunk D apply-every-patch, PARTIAL): D1 (dual-control fixture cleanup), D3 (DOA fixture band/role), D4 (replay cross-target arms) and D6 (per-route allow-list refusal arms) shipped with the D10 currency_refused predicate arm; story-9-7 now 30/30, predicates 14/14. COMPLETED 2026-09-07 (chunk D completion pass, bmad-code-review): D2 (weak/false-green assertions), D5 (direct-events door gaps), D7 (ack-replay-after-grant-change), D8 (arithmetic/lifecycle boundaries), D9 (pointer-latest + drift pins) and the remaining D10 sub-arms (ageing horizon boundary, CREDIT_NOTE_SUPERSEDED-in-AUDITED observability, offcutRetained raced/rolled-back) all shipped, plus a real code fix the new D8 arm exposed - `classifyDuplicate` (custody-ledger.ts) did not recognise the owned-lot / QC-hold constraint names, so a reused `disposal_id` surfaced as a raw 23505 500; the three names are now classified DUPLICATE_EVENT. Verification: tsc clean, eslint clean, prettier clean, story-9-7 38/38, story-9-8 13/13, story-1-9 6/6, schema-drift + predicates + 9-8 registry 175/175.
...
- [x] [Review][Patch] Weak/false-green assertions: the stock-class issue-bar arms accept any `>= 400` (a 404/500/RBAC error passes without proving `SOURCE_DOCUMENT_REQUIRED`); the AC 8 "closed order" arm accepts `[200, 409]` for the closure attempt so the order may never close and the closed-order sweep arm is unexercised; the ageing report arm reads the body before asserting status; `offcutRetained >= 1` cannot detect a doubled count; the clock-reconcile comparison floats `NUMERIC` strings through `Number()`; the offcut-holdings GET arm is satisfied by any disposed holding rather than the requested order's. FIXED 2026-09-07: exact `CROSS_ISSUE_BLOCKED` on the direct-event bar arm (the route arm pointed at `/api/v1/stock/issues`, a route this codebase never registered, so its 404 WAS the false-green - replaced with an explanatory comment and the seam-level arm); AC 8 captures the WHOLE custody balance so closure genuinely succeeds (200) and the sweep runs on a closed order, with `offcutRetained` asserted EXACTLY 1; the ageing `after` status is asserted before its body is read; the clock delta compares scaled integers via `scaledQty()`; the GET arm asserts the returned `holding_id`/`service_order_id` are the requested order's.
...
- [x] [Review][Patch] Direct-events door gaps for guards that are only proven at the route door: an `acquired` priced disposal by a non-finance actor (the finance gate arm only tries `returned`); the finance gate SITE sub-check (finance holder scoped to site B posting site A); the currency equality guards; the payload-site binding (site-B actor posting a site-A event); `CREDIT_NOTE_UNCITABLE`, `OFFCUT_NOT_RETAINED` and `CREDIT_NOTE_SUPERSEDED` door arms; the revaluation finance gate and revaluation `posted_by` pin on both doors; the events-door self-audit row for every `APPLIER_AUDITED_CODES` code (only SOD_VIOLATION is asserted today). FIXED 2026-09-07: all shipped. The finance-gate site sub-check needs a user whose jobwork write grant covers site A while the `finance_controller` grant is site-B-only - the pre-existing `otherSiteHeaders` fixture is site-B-only on BOTH, so the door's payload-site binding fires first and the sub-check is shadowed; the arm provisions exactly that split user. The events-door acknowledgment answers 201 (created), not the route's 200. The remaining self-audited codes reachable with the seeded fixtures (APPROVAL_REQUIRED, SOD_VIOLATION, OFFCUT_NOT_RETAINED, CREDIT_NOTE_UNCITABLE, CREDIT_NOTE_SUPERSEDED) are each asserted; CREDIT_NOTE_MISSING and APPROVAL_UNRESOLVED have no seeded-fixture trigger and stay covered at the route door only.
...
- [x] [Review][Patch] The acknowledgment-replay-after-grant-change branch is dead in tests (every ack fixture is wildcard-scoped): move a site-scoped grant between a successful ack and its retry and assert the 200 replay. Also pin the replay 200-vs-201 semantics for a retry that echoes its own id. FIXED 2026-09-07: a site-scoped `accounts_officer` acknowledges a note, the grant is moved to the other site, and the same-key retry still replays 200 with the SAME event_id - while a FRESH acknowledgment from the same caller now hits the 404 site collapse, proving the grant move took effect.
- [x] [Review][Patch] Untested arithmetic/lifecycle boundaries: a clock SHORTFALL (over-tolerance receipt leaves `clock_reconciled_qty < quantity` on the disposed row); `disposal_id` reuse across two different acquired holdings (QC-hold PK collision path); an in-range 14-digit rate times a large quantity overflowing `NUMERIC(18,4)` (the 22003 classification arms are dead code to the suite); a positive rate x tiny quantity rounding to `"0.0000"` (the P9 computed-value gate, end to end); acknowledging an earlier DELTA in a two-delta chain (P8 beyond the original). FIXED 2026-09-07: all shipped. The SHORTFALL arm over-receives 1040 against a 1000 challan and asserts `quantity 1030.000` with `clock_reconciled_qty 1000.000`. The `disposal_id`-reuse arm exposed a REAL BUG: `classifyDuplicate` knew neither `lot_master_pkey`/`uq_lot_master_lot_number` nor `qc_quality_hold_pkey`, so the second disposal died as an unclassified raw 500 - the three constraint names are now classified DUPLICATE_EVENT in custody-ledger.ts. The 22003 arm lives in the story-9-8 suite because the proposal row itself carries `proposed_value NUMERIC(18,4)` - the overflow surfaces at PROPOSE, not approval, and the arm proves classifyMoneyInsert maps it to a clean 400 with the row untouched and a normal proposal/approval still working afterward. The P9 arm uses quantity 0.001 at rate 0.0100 (computed value rounds to `"0.0000"`) and proves the lot still mints with NO credit note. The two-delta arm acknowledges delta1 after delta2 supersedes it and gets CREDIT_NOTE_SUPERSEDED, then acknowledges delta2.
- [x] [Review][Patch] The chunk B pointer-based latest-document fix is not mutation-worthy (serial revaluations cannot distinguish it from `created_at` ordering) and the schema-drift pins omit the new money CHECKs, the supersede FK and the standalone clock-column upgrade. FIXED 2026-09-07: the created_at of the original and delta1 are INVERTED (original later, delta1 earlier) and a third revaluation is asserted to chain off delta1 - both through `getLatestCreditNoteForHolding` directly and through the observable `supersedes_credit_note_id` - so a `ORDER BY created_at` revert fails the arm; schema-drift now pins `chk_job_work_credit_note_money` and `fk_job_work_credit_note_supersedes` by name, their BODIES (`rate >= 0 AND value >= 0`, `REFERENCES job_work_credit_note(credit_note_id)`) as negative arms in both the projection and init-db.sql, and the standalone `clock_reconciled_qty` column.
- [x] [Review][Patch] Small coverage holes: the ageing horizon boundary (90 days to `breached`, 89 to `due_within_30`, `beyond_90` dead for offcut); the `currency_refused` predicate arm (returned with a currency and no rate); `CREDIT_NOTE_SUPERSEDED` in `AUDITED_REJECTIONS` is unobservable (only its presence in the applier set is caught); the `offcutRetained` raced/rolled-back path. FIXED 2026-09-07: the ageing boundary arm backdates two holdings' `business_date` to TODAY-90 and TODAY-89 and asserts `breached` / `due_within_30` buckets with `beyond_90` count 0; the `currency_refused` predicate arm was already shipped with the initial chunk D pass; the CREDIT_NOTE_SUPERSEDED observability is now proven end to end through the events door (stale ack refused, audited row present); the offcutRetained raced/rolled-back path is pinned deterministically by running the sweep TWICE and asserting the exact counts 1 then 0 - a retried row is never counted again.
