...
| 2 | 13-2 bullet | 13-2 | DONE 2026-09-13 (ruling: the ERP "sometimes" learns platform GRNs): `legacy_received_qty` frozen on the line at first sync, counted in the tolerance band; the ERP's current `open_qty` never trusted for the band; excess it implies is echoed as `erp_receipt_overlap_qty` and logged; 3-4 and 2-9 arms | Every migrated PO that was partly received in the legacy system accepts a full second receipt on day one | Fix before the rehearsal; medium |
