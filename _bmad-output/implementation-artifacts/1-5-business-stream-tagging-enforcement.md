...
- [x] [Review][Patch] Rule applicability must use `envelope.metadata.occurred_at`, not server "now" [`src/compliance/business-stream.ts:71`] - Decision resolved 2026-07-19: evaluate applicability as-of `occurred_at` because that is compliance-correct for edge-synced and backdated events. `assertInventoryTagging` currently calls `findActiveTaggingRule(envelope.event_type)` with no date, so `findActiveTaggingRule` defaults to the current UTC date [`src/read/projections/business_stream_config.ts:122`]. Fix by deriving the local `YYYY-MM-DD` rule date from `envelope.metadata.occurred_at`, passing it to `findActiveTaggingRule`, and adding tests for a future-dated rule whose `occurred_at` is before and after `effective_from`.
- [x] [Review][Patch] Overlapping tagging-rule creation is not concurrency-safe [`src/api/v1/business-stream.ts:125`] - `findConflictingRule(...)` is called without the transaction `client`, so the overlap check reads on a separate pool connection outside the `BEGIN` snapshot; and even passing `client` would not stop two concurrent inserts under READ COMMITTED. The DB `UNIQUE (transaction_type, effective_from)` only catches an identical start date, so two overlapping-but-non-identical rules can both commit. Because the story ships no PATCH/DELETE for rules, `findActiveTaggingRule` then throws `TAGGING_CONFIG_CONFLICT` (500) for every inventory write of that type in the overlap window, unrecoverable through the API. High impact, low likelihood (admin-only, single-site pilot). Fix: pass `client` to `findConflictingRule` AND add a transaction-scoped guard (e.g. `pg_advisory_xact_lock` keyed on a hash of `transaction_type`, or a range-exclusion constraint).
- [x] [Review][Patch] Double `client.release()` on the 409 conflict path [`src/api/v1/business-stream.ts:136`] - the conflict branch releases the client then returns, and the `finally` at [`src/api/v1/business-stream.ts:171`] releases it again. node-postgres throws on a second release; that throw arises in `finally`, replaces the return with a rejection after the 409 response was already sent, and surfaces as an error against an already-sent response. Fix: remove the early `client.release()` in the conflict branch (the `finally` already covers it), or restructure to release once, mirroring the single-release shape of `src/api/v1/doa.ts`.
- [x] [Review][Patch] Undocumented `INVALID_TAG_VALUE` error code and 256-char cap, untested [`src/compliance/business-stream.ts:80-98`] - the length validation and its `INVALID_TAG_VALUE` 400 appear in neither the story tasks nor the story's declared stable error codes (`UNTAGGED_TRANSACTION`, `INVALID_BUSINESS_STREAM`, `TAGGING_RULE_CONFLICT`, `TAGGING_CONFIG_CONFLICT`); the underlying columns are payload JSONB / TEXT (unbounded), and no unit or integration test exercises either length branch. Fix: either remove the cap, or register `INVALID_TAG_VALUE` in the architecture's stable error codes and add a test per branch.
- [x] [Review][Patch] Stray repo-root artifact `PATCH_APPLICATION_REPORT.md` [`PATCH_APPLICATION_REPORT.md:1`] - committed at the repo root in `3090678`, it fails `git diff --check` (trailing whitespace on lines 3, 25, 61, 87, 137) and does not follow `FORMATTING_RULES.md`. Its own text also states integration tests were unavailable, contradicting the story's "86/86 green" claim. Fix: remove it (dev-log content belongs in the story's Dev Agent Record), or move and clean it.
