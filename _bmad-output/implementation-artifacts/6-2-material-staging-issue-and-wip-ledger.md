...
9. **Valuation is deliberately NOT coupled.** The Story 2.4 valuation seam is gated to `stream_type: 'inventory'` + `stock.received`/`stock.issued` (verified in `src/compliance/inventory-valuation.ts` lines 53-60). Production-stream movements call the stock helpers directly, exactly like Story 3.9 replenishment and Story 7.4 spare issue; do NOT call `applyValuationIssue` from this seam. Cross-stream valuation coherence is a platform gap logged to deferred-work (Task 9.6), not fixed here. **SUPERSEDED 2026-09-20 by owner ruling ("valuation must follow outgoing stock").** The original text above is kept for the record. The issue, backflush and return appliers now call `applyValuationOutflow` and `applyValuationReturn` in `src/compliance/inventory-valuation.ts` inside the event transaction, after the stock drain. WIP takes in EXACTLY the value that left valuation (owner default D1): the relieved block is split over the drained balance rows in drain order in SQL NUMERIC, each posting carries its own `valuation` share, `posting_value` equals that share and `unit_cost` is the share over the posting's whole quantity, so the unvalued part of an issue sits in WIP at zero. The cost per unit is the carrying cost of the locked valuation row (the running average until an NRV write-down, the written-down cost after it; the FIFO layer cost for a fifo item). Binding Decision 8 is superseded with it (owner default D4): the advisory unlocked read and the `WIP_COST_UNRESOLVED` refusal are gone from the handler and from both seams; an issue or backflush of stock with no cost basis goes through and the event (or the backflush line) is flagged `wip_cost_unresolved: true` so finance can find it. A return restores the share frozen on its source posting, net of what earlier returns put back, so the closing return restores exactly the rest. Backflush drains every stock row first, lines in ascending component SKU, and only then relieves the valuation rows in the same order, so two confirmations with opposite line order cannot deadlock. The frozen block is the audit record, not a replay input: no projection rebuild path exists today. Finished-goods valuation at completion is NOT built (deferred-work row 635, a known pilot limit). Deferred-work row 474 is closed.
...
| `WIP_COST_UNRESOLVED` | 409 | RETIRED 2026-09-20 (owner default D4): no longer raised; the issue goes through at zero WIP value and is flagged `wip_cost_unresolved` |
...
- Cross-stream valuation coherence: production-stream issues do not update `inventory_valuation` (the 3.9/7.4 precedent); logged as a platform decision. Superseded 2026-09-20 by owner ruling, see Binding Decision 9.
