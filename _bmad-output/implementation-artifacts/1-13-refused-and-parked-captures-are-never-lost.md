...
- [x] Task 8: Regression and quality gates (AC: all)
  ...
  - [ ] 8.5 NOT RUN BY THE AGENT (needs the deployed box and an operator-provided technician account). Staging (operator plus developer, after deploy): runbook row 2.10c with a technician-role account the operator provides. Rebuild the edge and app images, run `db:migrate`, and restart nginx after recreating the app container (cached upstream IP).
