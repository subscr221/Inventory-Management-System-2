...
**Re-review 2026-07-23 (adversarial pass over the fix commit `9fbdcf1`, baseline `07acb54`). Several patches from the prior pass are incomplete or introduced new issues.**

- [x] [Review][Decision] `UNIQUE(correlation_id)` guards nothing and the online create path lost retry idempotency — RESOLVED: User chose (b). Online create now accepts a client `Idempotency-Key` header (max 255 chars), stores it in `domain_events.idempotency_key`, replays the original gate event with 200 on retry (pre-check plus `uq_idempotency` race fallback), and never duplicates. Natural-key option rejected: same vehicle legitimately re-enters same gate same day. [src/api/v1/gate.ts, src/read/projections/gate_event.ts]
- [x] [Review][Decision] `binding_token` response field removed with no consumer migration — RESOLVED: User chose (a). Restored `binding_token` as a documented deprecated alias of `correlation_id` for contract stability; removal deferred to a versioned deprecation. [src/api/v1/gate.ts]
- [x] [Review][Decision] New `limit: 200` list cap has no pagination and hides the oldest unmatched worklist entries — RESOLVED: User chose (a). Added validated `offset` and `order` query params with clamped limit/offset in `listGateEvents`; the `binding=unmatched` worklist defaults oldest-first (AC3) while other listings stay newest-first; explicit `order` wins. [src/read/projections/gate_event.ts, src/api/v1/gate.ts]
- [x] [Review][Patch] `localYmd` IST fix is still timezone-dependent — FIXED: reads the shifted instant back with `getUTCFullYear`/`getUTCMonth`/`getUTCDate`, so `business_date` is host-timezone independent. [src/compliance/gate.ts:25-30]
- [x] [Review][Patch] Redundant double UNIQUE on `correlation_id` — FIXED: dropped the inline `UNIQUE` keyword; guarded `DO` block now also drops any legacy auto-named `gate_event_correlation_id_key` before adding `uq_gate_event_correlation_id`, in both canonical SQL and the init-db mirror. [read/projections/gate_event.sql, deploy/compose/init-db.sql]
- [x] [Review][Defer] `entered_at` is client-supplied with no upper bound and is the list sort key — a future-dated or bad-clock row pins to the top of every list. Deferred, pre-existing (validation not in this diff). [src/compliance/gate.ts:69]
- [x] [Review][Defer] Idempotent migration `ADD CONSTRAINT UNIQUE (correlation_id)` checks constraint name only, not data — aborts DB init if any pre-existing duplicate `correlation_id` data exists. Deferred, conditional and unlikely. [deploy/compose/init-db.sql:2141-2150]

**Re-review 2026-07-23 (second adversarial pass, over the uncommitted patch set fixing the first re-review). One patch introduced a critical regression, caught before merge.**

- [x] [Review][Patch] Idempotency-Key replay had no site/actor/body scope, letting a key collision return another site's gate event data — FIXED (CRITICAL): `getGateEventByIdempotencyKey` match is now verified against the requesting site (`site_id`) and the resubmitted body's `vehicle_reg_ext`/`gate_id`/`challan_photo_ref`/`po_ref_ext` before replay; any mismatch returns `409 IDEMPOTENCY_KEY_CONFLICT` instead of the foreign row. Covers both the pre-check and the concurrent-request race-fallback path. [src/api/v1/gate.ts]
- [x] [Review][Patch] Replay never compared the retried body to the original, so a corrected retry silently returned stale data — FIXED: same body-match check as above; a mismatched retry is rejected with 409 rather than silently replaying the first (possibly wrong) submission. [src/api/v1/gate.ts]
- [x] [Review][Patch] Replayed 200 responses wrote no audit_log entry — FIXED: a successful replay now writes an audit entry referencing the original event's `source_event_id`. [src/api/v1/gate.ts]
- [x] [Review][Patch] Duplicate `Idempotency-Key` header values silently took the first with no validation — FIXED (defensive; not reachable via a standard Node HTTP client, which coalesces repeat headers into one comma-joined string before this code runs): reject with 400 if the header arrives as an array of differing values. [src/api/v1/gate.ts]
- [x] [Review][Dismiss] `uq_idempotency` is globally unscoped across event types — superseded by the site/body verification above; a cross-domain key collision now also resolves to 409 rather than a data leak.
- [x] [Review][Defer] Cross-event-type `Idempotency-Key` collision (UUID-space collision outside `gate.entered`) rethrows the raw `DUPLICATE_EVENT` constraint error opaquely instead of a clean 409 — vanishingly unlikely, pre-existing `uq_idempotency` design. [src/events/store.ts:358-374]
- [x] [Review][Defer] Gate-event list response has no total-count/has-more signal — a caller that never sends `offset` cannot tell more rows exist beyond the 200 cap. Scope creep beyond the 3a decision (offset/order pagination); log for a future listing-contract pass. [src/api/v1/gate.ts, src/read/projections/gate_event.ts]
