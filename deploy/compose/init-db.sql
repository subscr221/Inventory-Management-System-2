...
-- Job-work offcut holding ledger (Story 9.6 revised, FR-JW-09/10). MIRROR COPY for first-boot
-- container init: the CANONICAL definition is read/projections/job_work_offcut_holding.sql, applied
-- by src/events/migrate.ts (npm run db:migrate) and the integration-test harness. Change both files
-- together; test/unit/schema-drift.test.ts fails the build when they diverge.
--
